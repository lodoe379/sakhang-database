<?php
echo "=== POSTGRESQL DATABASES (PORT 5432) ===\n";
try {
    $pgsql = new PDO('pgsql:host=127.0.0.1;port=5432;dbname=postgres', 'postgres', '123');
    $stmt = $pgsql->query("SELECT datname FROM pg_database WHERE datistemplate = false");
    $dbs = $stmt->fetchAll(PDO::FETCH_COLUMN);
    foreach ($dbs as $db) {
        echo "\nDB: $db\n";
        try {
            $dbConn = new PDO("pgsql:host=127.0.0.1;port=5432;dbname=$db", 'postgres', '123');
